...
Este script:
0. Detecta e pré-processa CSV bruto Maestro 187 (se necessário)
...
_PSEUDO_SALT = 'upa-rc-2026'

def _pseudo_id_s0(uid):
    key = _PSEUDO_SALT + ':' + str(uid).strip()
    return hashlib.sha256(key.encode('utf-8')).hexdigest()[:12]

def _decode_age_s0(raw):
    try:
        v = int(str(raw).strip())
        years = (v-300)/365.25 if v>=300 else (v-200)/12 if v>=200 else float(v)
    except Exception:
        return 'NI'
...
def _epi_week_s0(date_str):
    try:
        s = str(date_str).strip()
        if '/' in s:
            p = s.split('/'); d = _datetime.date(int(p[2]),int(p[1]),int(p[0]))
        elif '-' in s:
            p = s.split('-'); d = _datetime.date(int(p[0]),int(p[1]),int(p[2]))
        else: return None, None
        dow  = (d.weekday()+1) % 7
        sun  = d - _datetime.timedelta(days=dow)
        wed  = sun + _datetime.timedelta(days=3)
        yr   = wed.year
        jan1 = _datetime.date(yr,1,1)
        j1d  = (jan1.weekday()+1) % 7
        fs   = (jan1 - _datetime.timedelta(days=j1d) if j1d<=3
                else jan1 + _datetime.timedelta(days=7-j1d))
        se   = (sun - fs).days // 7 + 1
        return yr, max(1, min(53, se))
    except Exception:
        return None, None

def _find_col_s0(df, candidates):
    for c in df.columns:
        cl = c.lower().strip()
...
def _is_maestro187_raw(df):
    cols  = [c.lower().strip() for c in df.columns]
    has_d = any(c == 'data' for c in cols)
    has_u = any('usu' in c or 'código us' in c or 'codigo us' in c for c in cols)
    has_a = any('ano_epi' in c for c in cols)
    return has_d and has_u and not has_a

def step0_preprocess_if_needed(csv_path):
    """
    Detecta se o CSV é formato bruto Maestro 187.
    Se sim: pseudonimiza, decodifica idades, calcula SEs,
            remove duplicatas e agrega. Retorna novo caminho.
    Se não: retorna o caminho original sem alteração.
    """
    print("\n" + "=" * 60)
    ...
    if not _is_maestro187_raw(df):
        print("  → Formato agregado detectado. Step 0 pulado.")
        return csv_path

    print("  → Formato bruto Maestro 187. Pré-processando...")

    col_data = _find_col_s0(df, ['data'])
    df = df[df[col_data].str.match(r'\d{2}/\d{2}/\d{4}', na=False)].copy()
    print(f"  Linhas válidas: {len(df)}")

    if len(df) == 0:
        print("  ERRO: nenhuma linha válida.")
        return csv_path

    col_uc = _find_col_s0(df, ['código us','codigo us','cod us'])
    col_un = _find_col_s0(df, ['unidade de saúde','unidade de saude','unidade'])
    col_us = _find_col_s0(df, ['usuário','usuario'])
    col_sx = _find_col_s0(df, ['sexo'])
    col_fx = _find_col_s0(df, ['faixa etária','faixa etaria','faixa'])
    col_cp = _find_col_s0(df, ['c.e.p','cep'])
    col_cd = _find_col_s0(df, ['c.i.d.','cid_codigo','cid'])
    col_dc = _find_col_s0(df, ['descrição - c.i.d','descricao - c.i.d',
                                 'descrição','descricao','cid_descricao'])

    rows = []
    for _, r in df.iterrows():
        ds = r[col_data].strip()
        if not _re.match(r'\d{2}/\d{2}/\d{4}', ds): continue
        ae, se = _epi_week_s0(ds)
        if not ae: continue
        sx_raw = r[col_sx].strip() if col_sx else ''
        rows.append([
            ds, ae, se,
            r[col_uc].strip() if col_uc else '',
            r[col_un].strip() if col_un else '',
            _pseudo_id_s0(r[col_us].strip() if col_us else ''),
            ('MASCULINO' if sx_raw=='1' else 'FEMININO' if sx_raw=='2' else 'NI'),
            _decode_age_s0(r[col_fx].strip() if col_fx else ''),
            ''.join(c for c in (r[col_cp].strip() if col_cp else '') if c.isdigit()),
            r[col_cd].strip() if col_cd else '',
            r[col_dc].strip() if col_dc else '',
            1
        ])

    HDRS = ['data','ano_epi','semana_epi','unidade_codigo','unidade_nome',
            'pseudo_id','sexo','faixa_etaria','cep','cid_codigo',
            'cid_descricao','quantidade']
    out = pd.DataFrame(rows, columns=HDRS)
    print(f"  Linhas processadas: {len(out)}")

    out['_k'] = (out['data']+'|'+out['unidade_codigo']+'|'+
                 out['pseudo_id']+'|'+out['cid_descricao'])
    before = len(out)
    out = out[~out.duplicated('_k', keep='first')].drop(columns=['_k'])
    print(f"  Duplicatas removidas: {before-len(out)}")
...
def main():
    ...
    # Step 0 — detecta e pré-processa CSV bruto Maestro 187 se necessário
